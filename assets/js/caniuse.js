...
var canIUse = (function () {
    ...
    */
    ...
    /*  Configure the browsers you want to show here.
        ...
        * safari    - Apple Safari
    */
    var BROWSERS = ['chrome', 'firefox', 'edge', 'ie', 'opera', 'safari', 'ios_saf', 'android', 'op_mob', 'and_chr', 'and_ff'];

    //  Customise HTML here
    var TMPL_TITLE = '<div class="mihdan-caniuse__title">{title} <sup><a class="mihdan-caniuse__status">{status}</a></sup></div>', // feature title {title}
        TMPL_STATUS = '<p class="status">{status}</p>', // feature status (W3C Recommendation) {status}
        TMPL_DESCRIPTION = '<p class="mihdan-caniuse__description">{description}</p>', // description to user
        TMPL_DESKTOP_TITLE = '<div class="mihdan-caniuse__caption">Десктопы</div>', // desktop header
        TMPL_MOBILE_TITLE = '<div class="mihdan-caniuse__caption">Телефоны / Планшеты</div>', // mobile header
        TMPL_SUPPORT_WRAPPER = '<div class="mihdan-caniuse__agents"><ul class="agents">{items}</ul></div>', // support wrapper {items}
        ...
        TMPL_LEGEND = '<ul class="legend"><li>Supported:</li><li class="y"><span>Yes</span></li><li class="n">No</li><li class="a">Partially</li><li class="p">Polyfill</li></ul>',
        TMPL_FOOTER = '<p class="stats">Статистика с <a href="http://caniuse.com/#feat={feature}" target="_blank">caniuse.com</a></p>',
        TMPL_LOADING = '<div class="mihdan-caniuse__loader">Загрузка</div>',
        TMPL_ERROR = '<h1>Error</h1><p>Feature "{feature}" not found!</p>',
        TMPL_TABS = '<dl class="mihdan-caniuse-tabs"><dt class="selected">Notes</dt><dd class="selected">{notes}</dd><dt>Issues</dt><dd>{issues}</dd><dt>Resources</dt><dd>{resources}</dd><dt>Feedback</dt><dd>{feedback}</dd></dl>';
    ...
    function generateResults(feature) {
        ...
        results.description = feature.description; // feature name
    ...
    function generateHtml(results) {
        ...
        resultHtml = TMPL_TITLE.replace( '{title}', results.title ).replace( '{status}', results.status );
        resultHtml += TMPL_DESCRIPTION.replace('{description}', results.description);
    ...
    function generate(elementId) {
        ...
        if (typeof elementId === "undefined") {
            $canIUse = document.querySelectorAll('.mihdan-caniuse');
        } else {
    ...
    return {
        render: populate,
        dataLoaded: function (data) { //console.log(data);
            canIUseData = data;
            if (populateOnLoad) {
...
jQuery( function ( $ ) {
    ...
    $(function(){
        $( document.body ).on( 'click', 'dl.mihdan-caniuse-tabs dt', function(){
            $(this)
